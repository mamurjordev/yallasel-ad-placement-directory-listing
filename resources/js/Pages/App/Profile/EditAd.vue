<template>
    <div class="bg-gray-50">
        <section v-if="success" class="fixed w-full mx-auto">
            <div
                class="mx-auto flex justify-between bg-green-400 text-gray-100 container px-5 py-2 rounded-md shadow-xl"
            >
                <p class="font-semibold">
                    ...
                    <inertia-link
                        ...
                        href="/my-ads"
                        >Return to my ads</inertia-link
                    >
                </p>
                <button @click="success = false">&#215;</button>
            </div>
        </section>
        <!-- Header -->
        <section
        ...
        >
            <div class="flex justify-start items-baseline space-s-5 mb-5">
                ...
                <!-- <inertia-link
                    href=""
                    class="hidden md:block underline text-sm"
                    >{{ __("Price information") }}</inertia-link
                > -->
            </div>
            ...
            <div class="mt-3 bg-gray-100 p-4 w-10/12 pe-20">
                ...
                <section
                    v-if="categoryProperties.length > 0"
            ...
            <div class="mt-3">
                <label for="" class="text-sm font-medium"
                    >{{ __("Heading") }} <span class="text-red-700">*</span>
                </label>
                <input
                    type="text"
            ...
            <div class="mt-3">
                <label for="" class="text-sm font-medium">
                    {{ __("Text") }} <span class="text-red-700">*</span>
                </label>
                <vue-editor
            ...
            <div class="mt-3">
                <label for="" class="text-sm font-medium">
                    {{ __("Price") }} <span class="text-red-700">*</span>
                </label>
        ...
        <section
            ...
            class="fixed z-10 top-0 start-0 w-full h-full overflow-auto font-roboto"
        >
            <div class="container mx-auto bg-white mt-5 py-5">
                <div
                    class="cursor-pointer underline font-light text-right px-5 mb-6"
                    @click="openPreview"
                >
                    {{ __("close") }}
                </div>
                <div class="bg-gray-100">
                    ...
                    <div class="flex justify-items-start space-s-3 mt-10 mx-5">
                        <!-- <img v-for="previewImage in previewImages" :key="previewImage.id" :src="previewImage" alt="" class="w-12 h-10" /> -->
                        <img
                            v-if="previewImages[0]"
                            :src="previewImages[0]"
                            alt=""
                            class="w-12 h-10"
                            @click="mainPreview = previewImages[0]"
                        />
                        <img
                            v-if="previewImages[1]"
                            :src="previewImages[1]"
                            alt=""
                            class="w-12 h-10"
                            @click="mainPreview = previewImages[1]"
                        />
                        <img
                            v-if="previewImages[2]"
                            :src="previewImages[2]"
                            alt=""
                            class="w-12 h-10"
                            @click="mainPreview = previewImages[2]"
                        />
                        <img
                            v-if="previewImages[3]"
                            :src="previewImages[3]"
                            alt=""
                            class="w-12 h-10"
                            @click="mainPreview = previewImages[3]"
                        />
                        <img
                            v-if="previewImages[4]"
                            :src="previewImages[4]"
                            alt=""
                            class="w-12 h-10"
                            @click="mainPreview = previewImages[4]"
                        />
                        <img
                            v-if="previewImages[5]"
                            :src="previewImages[5]"
                            alt=""
                            class="w-12 h-10"
                            @click="mainPreview = previewImages[5]"
                        />
                    </div>
                </div>
                <div class="px-5 mt-3">
                    <h5 class="text-2xl">{{ form.heading }}</h5>
                    <div class="text-gray-600 font-light text-sm mb-5 mt-2">
                        {{ __("For sale by") }} {{ form.name }}
                    </div>
                    <p
                        v-html="form.text"
                ...
                <div
                    class="text-center bg-gray-50 py-2 text-gray-500 mt-10 mb-5"
                >
                    <div class="text-sm font-light">
                        {{ __("Email or call") }}
                        <span class="font-medium">{{ form.email }}</span>
                    </div>
                    <div class="text-2xl font-light mt-1">{{ form.phone }}</div>
                </div>
                <div
                    class="cursor-pointer underline text-center pb-4"
                    @click="openPreview"
                >
                    {{ __("Close preview") }}
                </div>
...
export default {
    ...
    // },
    created() {
        this.form.is_individual = this.post.is_individual;
        this.form.phone = this.post.phone;
        this.form.hidePhoneInAd = this.post.hidePhoneInAd;
        this.form.category_id = this.post.category_id;
        this.form.reason = this.post.reason;
        this.form.price = this.post.expected_price;
        this.form.zipcode = this.post.zipcode;
        this.form.city_id = this.post.category_id;
        this.form.region_id = this.post.region_id;
        this.form.heading = this.post.title;
        this.form.text = this.post.detail;
        this.form.payment = this.post.payment;
        this.form.terms = this.post.terms;

        this.form.postProperties = this.postAttributes.post_attribute
            ? JSON.parse(this.postAttributes.post_attribute)
            : {};

        this.getCatProperties();

        this.postImages.forEach(element => {
            var image = "/media/posts/" + element.image;
            this.previewImages.pop();
            this.form.images.pop();
            this.previewImages.unshift(image);
            this.form.images.unshift(element.image);
        });
    },
    watch: {
        "form.images"() {
            this.form.image1 = this.form.images[0];
            this.form.image2 = this.form.images[1];
            this.form.image3 = this.form.images[2];
            this.form.image4 = this.form.images[3];
            this.form.image5 = this.form.images[4];
            this.form.image6 = this.form.images[5];
        },
        "form.category_id"() {
            console.log("ok");
            this.getCatProperties();
    ...
    methods: {
        redirect() {
            this.$inertia.visit("/edit-ad/" + this.post.id);
        },
        async getCatProperties() {
            await axios
        ...
        submit() {
            ...
            if (this.form.terms == true) {
                this.$inertia.post("/post/" + this.post.id, data);
                this.success = true;
                this.successMsg = "Post Updated Successfully";
            } else {
                this.terms = true;
            }
        ...
        addToImages(e) {
            this.form.images.pop();
            this.form.images.unshift(e.target.files[0]);
