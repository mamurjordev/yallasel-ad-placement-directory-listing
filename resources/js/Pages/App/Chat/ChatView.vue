...
export default {
    ...
    props: ["user", "reciever", "currentUserId", "isDevice"],
    ...
    data() {
        return {
            ...
            removeUserId: "",
            removeUsers: [],
            roomActions: [{ name: "deleteRoom", title: "Delete" }],
            menuActions: [{ name: "deleteRoom", title: "Delete" }],
    ...
    methods: {
        ...
        //                     messagesRef(doc.id).onSnapshot(messages => {
            ...
            await roomsRef
                ...
                .then(querySnapshot => {
                    if (querySnapshot.docs.length > 0) {
                        querySnapshot.forEach(doc => {
                            if (
                                doc.data().users.includes("" + this.reciever.id)
                            ) {
                                this.roomId = doc.id;
                            }
                        });
            ...
            Object.keys(roomList).forEach(key => {
                ...
                const roomAvatar =
                    roomContacts.length === 1 && roomContacts[0].avatar
                        ? "/media/user/" + roomContacts[0].avatar
                        : "/media/user/default.svg";
        ...
        fetchMessages({ room, options = {} }) {
            ...
            if (this.startMessages)
                query = query.startAfter(this.startMessages);
            ...
            query.get().then(messages => {
                ...
                if (this.startMessages)
                    listenerQuery = listenerQuery.startAfter(
                        this.startMessages
                    );
                if (this.endMessages)
        ...
        markMessagesSeen(room, message) {
            if (
                ...
                (!message.data().seen ||
                    !message.data().seen[this.currentUserId])
            ) {
            ...
            await roomsRef
                ...
                .then(querySnapshot => {
                    if (querySnapshot.docs.length > 0) {
                        ...
                        roomsRef.add({
                            users: ["" + this.user.id, "" + this.reciever.id],
                            lastUpdated: new Date()
                        });
                    }
                });
        ...
        async addRoomUser() {
            ...
            await roomsRef.doc(this.inviteRoomId).update({
